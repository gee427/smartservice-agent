...
一套「本地 AI 聊天平台」（2026-09-02 起，原智能客服定位已去客服化）：用户在网页上与通用 AI 助手自由聊天，
天气 / 数学计算两个场景由确定性关键词直连工具 Agent；运营人员通过管理后台监控平台健康与运营指标。
全链路覆盖商用项目的完整环节。
...
| LM Studio | 最新 | 本地 LLM 服务（**可挂**：服务可启动，但对话会降级为错误提示，无兜底问答路径） |
...
### 4.1 聊天端（用户视角）
- **功能**：SSE 流式打字机输出、意图路由标签（通用对话 / 天气查询 / 数学计算）、多轮上下文（localStorage 持久化会话，刷新不丢）、快捷问题按钮。
- **试试这些**：`北京今天天气怎么样？`（Weather）· `帮我算一下 15*23+8 等于多少`（Calc）· `写一首关于秋天的短诗`（Chat）· `把「你好世界」翻译成英文`（Chat）。
...
| 聊天一直回复「AI 服务暂时不可用」 | LM Studio 没起/模型未加载 | 启动 LM Studio 并加载模型（现无无 LLM 兜底问答路径） |
